...
export async function processPostingPipeline(job: Job<PostingPipelineJobData>) {
  ...
  log.info({ jobId: job.id, scheduleId, accountId, platform }, 'Starting automated posting pipeline');
  ...
  try {
    ...
    log.info({ contentId: content.id, postUrl, durationMs }, '✅ Pipeline completed: posted successfully');
    ...
    log.error({ contentId: content.id, err: error.message, durationMs }, '❌ Pipeline failed; will retry if attempts remain');
